...
- **Adaptive frame rate (default)** — the receiver's audio ACKs report how many blocks/s it actually decodes; the sender automatically walks its frame interval (50–400 ms ladder) toward the fastest rate the receiver keeps up with. Blasting faster than the camera decodes only shortens each code's on-screen dwell and *lowers* goodput. Fixed intervals down to 50 ms remain selectable. (A 2×2 multi-QR grid and an extra-dense preset were field-tested and removed: real-world cameras could not resolve them reliably.)
- **Frame-change beacon + stability gate** — the receiver skips stale duplicates *and* mid-swap torn frames (a region is decoded only once its 16×16 luminance signature is stable across two consecutive camera frames and differs from the last decoded content; the gate pauses itself if it ever starves decoding).
...
- **Screen wake locks** on both sides, camera zoom controls, native `CompressionStream` compression when available, and **focus/exposure lock**: after the first successful decode the receiver pins the camera's focus and exposure at their current (working) values where the device allows it, preventing the mid-transfer autofocus hunting that shows up as minutes-long stalls.
...
1. Choose a file.
2. Pick a quality preset (**Balanced** is the default — see [Choosing a preset](#choosing-a-preset)). Leave the frame interval on **Auto** — with the ACK mic on, the sender finds the fastest rate the receiver's camera actually sustains; pick a fixed interval only if you're running without audio ACKs.
...
Theoretical rate at Balanced is ~9.7 KB/s at 10 frames/s, more at shorter intervals. Real-world throughput depends on the camera — which is why **Auto** rate is the default: it measures the receiver's true decode rate from the ACK stream and converges on the sweet spot. On a fixed interval, if the receiver's "rejected" count climbs or progress stalls, slow down before reaching for the Robust preset. (Three throughput experiments were field-tested and removed: an extra-dense version-30 preset and a 2×2 multi-QR grid, which typical cameras never decoded, and a color-multiplexed "2×" mode packing two QR codes into the red and blue channels of one image, which was ~2× *slower* — camera Bayer sensors sample R and B at only a quarter of the pixels, so each color channel carries about half the linear resolution of the mono image, and crosstalk cuts contrast further. The lesson generalizes: the camera's pixels-per-module budget is the binding constraint, and any scheme that shrinks the effective module size loses.)
